...
export const showContextMenu = (e, path, isDirectory) => {
    ...
    const items = [
        ...
        { divider: true },
        { label: 'Rename', icon: 'edit', onClick: () => renameItem(path) },
...
const renameItem = async (oldPath) => {
    ...
    window.gxPrompt('Rinomina', 'Nuovo nome', oldName, async (newName) => {
        if (!newName || newName === oldName) return;
        
        // Calcolo del nuovo path
        const parentDir = oldPath.substring(0, oldPath.lastIndexOf(oldPath.includes('\\') ? '\\' : '/'));
        const pathDelimiter = oldPath.includes('\\') ? '\\' : '/';
        const newPath = `${parentDir}${pathDelimiter}${newName}`;

        const res = await window.electronAPI.fsRename(oldPath, newPath);
        if (res && res.error) {
            window.gxAlert('Errore Rinomina', res.error, 'error');
        } else {
            window.gxToast('Rinomina completata', 'success');
            // Se il file rinominato era aperto nell'editor, chiudiamolo o aggiorniamolo
            // Per ora facciamo solo un refresh dell'explorer
            refreshWorkspace();
            if (window.refreshWorkspace) window.refreshWorkspace(); // Per workspace.js compatibility
        }
    });
};
